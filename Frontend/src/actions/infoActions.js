import { FETCH_INFO_DETAIL, FETCH_INFO_FIND, FETCH_INFO_LIST } from "./types";
// BOARD => Get / Post / Put / Delete => RestFul
import axios from "axios";

export const fetchInfoList = (cno, page) => (dispatch) => {
  axios
    .get("http://localhost/info/list_react", {
      params: {
        cno: cno,
        page: page,
      },
...
export const fetchInfoDetail = (no) => (dispatch) => {
  axios
    .get("http://localhost/info/detail_react", {
      params: {
        no: no,
      },
    })
...
export const fetchInfoFind = (fd) => (dispatch) => {
  axios
    .get("http://localhost:3355/info/find", {
      params: {
        fd: fd,
      },
    })
